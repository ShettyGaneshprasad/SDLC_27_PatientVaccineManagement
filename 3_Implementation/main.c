 ...
 * @file main.c
 * @author AnandBotkule
 * @brief Main starting block of the programme
 * @version 0.1
 * @date 2021-08-24
 * 
...
int main()
{
    ...
    FILE *rt = NULL;
    patient *start = NULL;
    error_t patient_indexfile = patient_indexFile("INDEX.DAT", "rb+", &f);

    error_t recordFile = patient_openFile("RECORD.DAT", "rb+", &r);
    ...
    patient *(*fptr_type1)(patient *, int, char *, char *, char *, float, float, int, char *, insured, vaccine) = create_patientid;
    error_t (*fptr_type2)(patient *, int, char *, char *, char *, float, float, int, char *, insured, vaccine) = insert_patientrecord;
    error_t (*fptr_type3)(patient *) = display_patientid;
    error_t (*fptr_type4)(patient *, int, patient *, int *) = find_by_patientid;
    error_t (*fptr_type5)(patient *, int, int, int) = update_patientrecord;
    error_t (*fptr_type6)(patient *, FILE **, int *) = patient_saveFile;
    int choice;
    while (1)
    {
        ...
        if (choice >= 1 && choice <= 10)
        {
            ...
            if (choice == 1)
            {
                ...
                rewind(f);
                error_t code = validate_patientid(&f, id);
                if (code == ID_EXISTS)
                {
            ...
            if (choice == 2)
            {
                ...
                //step 1 validation to check existing ids
                rewind(f);
                error_t file_code = validate_patientid(&f, id);
            ...
            if (choice == 6)
            {
                error_t code = patient_readFile(&r);
            }
            ...
            if (choice == 7)
            {
                ...
                if (found == 1)
                {
                    start = delete_patientid(start, id);
                    error_t del_code_2 = delete_patientrecords_file(&r, &rt, id);
                    ...
                    patient_openFile("INDEX.DAT", "rb+", &f);
                }
                else
                {
                    error_t del_code = delete_patientrecords_file(&r, &rt, id);
