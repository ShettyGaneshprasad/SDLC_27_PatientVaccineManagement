...
patient *patient_loadData(patient *start, FILE **fptr)
{
    ...
    while (fread(&rec, sizeof(rec), 1, *fptr) == 1)
    {
        ...
        else
        {
            ...
            insert_patientrecord(start, rec.uniq_id, rec.firstname, rec.lastname, rec.aadhar_num, rec.height, rec.weight, rec.age, rec.date_of_vaccine, rec.insurance, rec.shot);
            free(new_box);
        }
    }
    return start;
